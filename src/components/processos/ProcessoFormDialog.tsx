...
export function ProcessoFormDialog({
  ...
  const formaAtual = formasPagamento.find((f) => f.codigo === form.forma_pagamento);

  const isCredito =
    ["credito", "cartao", "cartao_credito"].includes(form.forma_pagamento) ||
    formaAtual?.tipo === "credito";

  const showParcelas =
    !!formaAtual?.abre_parcelas ||
    isCredito ||
  ...
  const handlePercentChange = (val: number[]) => {
    ...
    setForm((f) => ({
      ...f,
      percentual_empresa: emp,
      percentual_profissional: 100 - emp,
    }));
  };
    ...
    <>
      <div className="space-y-2">
        ...
        <Select value={form.forma_pagamento} onValueChange={handleFormaPagamentoChange}>
          <SelectTrigger>
            <SelectValue placeholder={formasPagamentoLoading ? "Carregando..." : "Selecionar"} />
          </SelectTrigger>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh]">
        <DialogHeader>
          <DialogTitle>
            {isEditing ? "Editar Processo" : "Novo Processo Individual"}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="grid gap-4 max-h-[65vh] overflow-y-auto pr-2">
            ...
            <div className="space-y-2">
              ...
              {profissionais.length === 0 && (
                <p className="text-xs text-muted-foreground">
                  Cadastre profissionais na aba Profissionais primeiro.
                </p>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setForm((f) => ({
                    ...f,
                    cliente_telefone: formatPhone(e.target.value),
                  }))
                }
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setForm((f) => ({
                    ...f,
                    valor_total: formatCurrencyInput(e.target.value),
                  }))
                }
                placeholder="0,00"
              />
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              <Button
                type="button"
                variant={tipoPagamento === "entrada" ? "default" : "outline"}
                className="w-full"
              ...
              <Card>
                <CardContent className="pt-4 space-y-4">
                  <p className="text-sm font-medium text-muted-foreground">
                    O cliente dará uma entrada agora. O restante será lançado depois conforme o acordo.
                  </p>
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setForm((f) => ({
                            ...f,
                            valor_entrada: formatCurrencyInput(e.target.value),
                          }))
                        }
                        placeholder="0,00"
                      />
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <span className="text-muted-foreground">
                    Grupo Excellence ({form.percentual_empresa}%):
                  </span>
                  <p className="text-lg font-semibold text-primary">
                    {formatCurrency((valorNumerico * form.percentual_empresa) / 100)}
                  </p>
                </div>

                <div>
                  <span className="text-muted-foreground">
                    Profissional ({form.percentual_profissional}%):
                  </span>
                  <p className="text-lg font-semibold">
                    {formatCurrency((valorNumerico * form.percentual_profissional) / 100)}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Select
                value={form.comercial_id || "nenhum"}
                onValueChange={(v) =>
                  setForm((f) => ({
                    ...f,
