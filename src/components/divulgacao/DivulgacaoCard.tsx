    ...
    <Card className="group transition-shadow hover:shadow-md border bg-card">
      {divulgacao.imagem_url && (
        <div className="w-full h-32 overflow-hidden rounded-t-lg">
          <img
            src={divulgacao.imagem_url}
            alt={divulgacao.titulo}
      ...
      <CardContent className="p-3 space-y-2">
        ...
        <span
          className={`inline-block text-[10px] font-semibold px-2 py-0.5 rounded-full ${
            CATEGORIA_COLORS[divulgacao.categoria] ?? "bg-muted text-muted-foreground"
          }`}
        >
        ...
        {divulgacao.descricao && (
          <p className="text-xs text-muted-foreground line-clamp-2">{divulgacao.descricao}</p>
        )}
        ...
        <div className="flex items-center justify-between pt-1">
          ...
          <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <Button
              ...
              onClick={() => onDelete(divulgacao.id)}
            >
