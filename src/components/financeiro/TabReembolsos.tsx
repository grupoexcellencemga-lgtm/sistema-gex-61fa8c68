...
export const TabReembolsos = ({ mes, ano }: { mes: number; ano: number }) => {
  ...
  const getComprovantes = (item: any) => {
    const lista = Array.isArray(item?.comprovantes_urls)
      ? item.comprovantes_urls
      : [];
    if (lista.length > 0) return lista;
    if (item?.comprovante_url)
      return [{ url: item.comprovante_url, nome: "Arquivo anexado" }];
    return [];
  };
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setEditingItem(null);
      toast({
        title: editingItem ? "Reembolso atualizado" : "Reembolso registrado",
      });
    },
  ...
  const handleSave = async () => {
    ...
    let comprovantes_urls: Array<{ url: string; nome: string }> =
      getComprovantes(editingItem);
  ...
  // Filter by month
  const reembolsosMes = reembolsos.filter((r: any) =>
    isInMonth(r.data_despesa, mes, ano),
  );

  // Saldo por pessoa
  const saldoPorPessoa: Record<
    string,
    { nome: string; tipo: string; pendente: number; pago: number }
  > = {};
  reembolsosMes.forEach((r: any) => {
    const key = r.pessoa_id || r.pessoa_nome;
    if (!saldoPorPessoa[key])
      saldoPorPessoa[key] = {
        nome: r.pessoa_nome,
        tipo: r.pessoa_tipo,
        pendente: 0,
    ...
    if (r.status === "pendente")
      saldoPorPessoa[key].pendente += Number(r.valor);
    else saldoPorPessoa[key].pago += Number(r.valor);
  });
  const saldoList = Object.values(saldoPorPessoa)
    .filter((s) => s.pendente > 0 || s.pago > 0)
    .sort((a, b) => b.pendente - a.pendente);

  const totalPendente = reembolsosMes
    .filter((r: any) => r.status === "pendente")
    .reduce((s: number, r: any) => s + Number(r.valor), 0);
  const totalPago = reembolsosMes
  ...
  const filtered = reembolsosMes.filter((r: any) => {
    if (
      filterPessoa &&
      !r.pessoa_nome.toLowerCase().includes(filterPessoa.toLowerCase())
    )
      return false;
    ...
    <div className="space-y-6">
          ...
          <CardContent className="pt-4 pb-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-lg font-bold">
                {reembolsos.filter((r: any) => r.status === "pendente").length}
              </p>
            </div>
          </CardContent>
        </Card>
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground capitalize">
                      {s.tipo === "comercial" ? "Vendedor" : "Profissional"}
                    </p>
                  </div>
                ...
                paginated.map((r: any) => (
                  <TableRow key={r.id}>
                    ...
                    <TableCell className="text-sm">
                      {formatDate(r.data_despesa)}
                    </TableCell>
                    <TableCell className="text-sm">
                      {formatDate(r.data_reembolso)}
                    </TableCell>
                    <TableCell className="text-sm">
                      {r.contas_bancarias?.nome || "—"}
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <div className="flex gap-1">
                        ...
                        <Button
                          ...
                          onClick={() => {
                            if (confirm("Remover este reembolso?"))
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>
              {editingItem ? "Editar Reembolso" : "Novo Reembolso"}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
              ...
              <Select
                ...
                onValueChange={(v) => {
                  ...
                  setForm((f) => ({
                    ...
                    pessoa_id: v,
                    pessoa_nome: p?.nome || "",
                    pessoa_tipo: p?.tipo || "profissional",
                  }));
                }}
              >
                <SelectTrigger>
                  <SelectValue placeholder="Selecione quem adiantou..." />
                </SelectTrigger>
                <SelectContent>
                  {pessoas.map((p) => (
                    <SelectItem key={p.id} value={p.id}>
                      ...
                      <span className="text-muted-foreground ml-1">
                        ({p.tipo === "comercial" ? "Vendedor" : "Profissional"})
                      </span>
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <div>
              ...
              <Input
                placeholder="Ex: Almoço equipe dia 20/03"
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setForm((f) => ({ ...f, data_despesa: e.target.value }))
                  }
                />
              </div>
            </div>
            ...
            <div>
              ...
              {getComprovantes(editingItem).length > 0 && (
                <div className="space-y-2 mb-2">
                  {getComprovantes(editingItem).map(
                  ...
                  >
                    ...
                    Remover todos os comprovantes ao salvar
                  </Button>
                </div>
              )}
              <Input
      ...
      <Dialog open={pagarDialogOpen} onOpenChange={setPagarDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Pagar Reembolso</DialogTitle>
          </DialogHeader>
          {payingItem && (
            <div className="space-y-4">
              <div className="p-3 rounded-lg bg-muted/50">
                <p className="text-sm">
                  <strong>{payingItem.pessoa_nome}</strong>
                </p>
                <p className="text-sm text-muted-foreground">
                  {payingItem.descricao}
                </p>
                <p className="text-lg font-bold text-primary mt-1">
                  {formatCurrency(Number(payingItem.valor))}
                </p>
              </div>
              <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                  ...
                  >
                    ...
                    <SelectContent>
                      {loadingFormasPagamento ? (
                        <SelectItem
                          value="carregando_formas_pagamento"
                          disabled
                        >
                          Carregando formas...
                        </SelectItem>
                ...
                <div>
                  ...
                  <Input
                    type="date"
                    value={payForm.data_reembolso}
                    onChange={(e) =>
                      setPayForm((f) => ({
                        ...f,
                        data_reembolso: e.target.value,
                      }))
                    }
                  />
                </div>
