...
export const TabComissoes = ({ mes, ano }: { mes: number; ano: number }) => {
  ...
  const pagarComissao = useMutation({
    mutationFn: async (comissao: any) => {
      const valor =
        Number(pgForm.valor) ||
        Number(comissao.valor_comissao) - Number(comissao.valor_pago);
      ...
      if (catExist) {
        ...
        await supabase.from("categorias_despesas").insert({
          id: newCatId,
  ...
  const excluirComissao = useMutation({
    mutationFn: async (comissao: any) => {
      if (comissao.status === "pago") {
        throw new Error(
          "Não é possível excluir uma comissão já paga. Primeiro desfaça o pagamento."
        );
      }

      const confirmado = window.confirm(
        `Deseja excluir esta comissão pendente?\n\nAluno: ${
          comissao.alunos?.nome || "—"
        }\nValor: ${formatCurrency(Number(comissao.valor_comissao))}`
      );
  ...
  const comissoesMes = comissoes.filter((c: any) =>
    isInMonth(c.created_at, mes, ano)
  );

  const buildHierarchy = (comercialId: string) => {
    const cms = comissoesMes.filter((c: any) => c.comercial_id === comercialId);

    const produtoMap: Record<
      string,
      {
        nome: string;
        turmas: Record<string, { nome: string; comissoes: any[] }>;
      }
    > = {};
    ...
    cms.forEach((c: any) => {
      ...
      if (!produtoMap[prodId]) {
        produtoMap[prodId] = { nome: prodNome, turmas: {} };
      }

      if (!produtoMap[prodId].turmas[turmaId]) {
        produtoMap[prodId].turmas[turmaId] = {
          nome: turmaNome,
          comissoes: [],
        };
      }
  ...
  const getVendedorTotals = (comercialId: string) => {
    ...
    const total = cms.reduce(
      (s: number, c: any) => s + Number(c.valor_comissao),
      0
    );

    const pago = cms.reduce(
      (s: number, c: any) => s + Number(c.valor_pago),
      0
    );
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-semibold">
        Comissões por Vendedor (5% por matrícula)
      </h3>
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="py-12 text-center text-muted-foreground">
            Nenhum vendedor ativo. Cadastre vendedores na página "Vendedores" no
            menu lateral.
          </CardContent>
        </Card>
      ) : (
        comerciais.map((com: any) => {
          ...
          return (
            <Card key={com.id} className="overflow-hidden">
              <button
                onClick={() => {
                  setExpandedVendedor(isVendedorExpanded ? null : com.id);
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {totals.vendas} venda{totals.vendas !== 1 ? "s" : ""} ·{" "}
                      {totals.pendentes} pendente
                      {totals.pendentes !== 1 ? "s" : ""}
                    </p>
                  </div>
                </div>
                ...
                <CardContent className="pt-0 space-y-3">
                  ...
                  {Object.keys(hierarchy).length === 0 ? (
                    <p className="text-sm text-muted-foreground text-center py-4">
                      Nenhuma comissão vinculada. Vincule este vendedor ao criar
                      uma matrícula.
                    </p>
                  ) : (
                    Object.entries(hierarchy).map(([prodId, prod]) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {isProdExpanded && (
                            <CardContent className="pt-0 space-y-3">
                              {Object.entries(prod.turmas).map(
                                ([turmaId, turma]) => {
                                  const turmaKey = `${prodKey}-${turmaId}`;
                                  const isTurmaExpanded =
                                    expandedTurma === turmaKey;

                                  const turmaTotal5 = turma.comissoes.reduce(
                                    (s: number, c: any) =>
                                      s + Number(c.valor_comissao),
                                    0
                                  );

                                  const turmaPago = turma.comissoes.reduce(
                                    (s: number, c: any) =>
                                      ...
                                      >
                                        <div className="flex items-center gap-2">
                                          ...
                                          <Badge
                                            ...
                                            className="text-xs"
                                          >
                                            {turma.comissoes.length} aluno
                                            {turma.comissoes.length !== 1
                                              ? "s"
                                              : ""}
                                          </Badge>
                                        </div>
                                                        ...
                                                        c.id ? (
                                                          <div className="flex flex-col gap-2 min-w-[240px]">
                                                            ...
                                                            <div className="flex gap-1">
                                                              <Button
                                                                ...
                                                                onClick={() =>
                                                                  pagarComissao.mutate(
                                                                    c
                                                                  )
                                                                }
                                                                disabled={
                                                                  pagarComissao.isPending
                                                                }
                                                              >
                                                                {pagarComissao.isPending ? (
                                                          ...
                                                          <div className="flex items-center gap-1">
                                                            ...
                                                            <Button
                                                              ...
                                                              onClick={() =>
                                                                excluirComissao.mutate(
                                                                  c
                                                                )
                                                              }
                                                              disabled={
                                                                excluirComissao.isPending
                                                              }
