...
import { statusMap } from "../processos/processosUtils";
import { LancamentosEmpDialog } from "./LancamentosEmpDialog";
...
export function ProcessoEmpTable({
          ...
          <TableBody>
            {processos.length === 0 ? (
              ...
              processos.map((p: any) => {
                ...
                const progresso = totalP > 0 ? Math.min(100, Math.round((recebido / totalP) * 100)) : 0;
                  ...
                  <TableRow key={p.id} className="cursor-pointer hover:bg-muted/50" onClick={() => onSelect(p)}>
                    ...
                    <TableCell onClick={(e) => e.stopPropagation()}>
                      <div className="flex gap-1 justify-end">
                        <LancamentosEmpDialog processo={p} contas={contas} />
                        {p.status === "aberto" && (
                          <>
