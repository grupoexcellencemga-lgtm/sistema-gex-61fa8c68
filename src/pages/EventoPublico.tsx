...
import { Loader2, Calendar, MapPin, Users, ChevronDown, ChevronUp } from "lucide-react";
import { Button } from "@/components/ui/button";
import { formatDate } from "@/lib/formatters";
import { Secao, Palestrante, AgendaItem, Depoimento, FaqItem } from "@/components/eventos/pagina-publica/types";

const formatValor = (v: number) =>
  v.toLocaleString("pt-BR", { style: "currency", currency: "BRL" });

function SecaoHero({ evento, dados, onInscrever }: { evento: any; dados: any; onInscrever: () => void }) {
  return (
    <section className="relative min-h-[60vh] flex items-end">
      ...
      <div className="relative z-10 w-full max-w-4xl mx-auto px-6 pb-12 pt-24">
        ...
        <h1 className="text-4xl md:text-5xl font-bold text-white leading-tight mb-3" style={{ textWrap: "balance" }}>
          {evento.nome}
        </h1>
        {dados?.subtitulo && (
...
function SecaoSobre({ dados }: { dados: any }) {
    ...
    <section className="py-16 px-6">
      <div className="max-w-4xl mx-auto">
        ...
        <div className={`gap-10 ${dados.imagem_url ? "grid md:grid-cols-2" : ""}`}>
          ...
          {dados.imagem_url && (
            <img src={dados.imagem_url} alt="" className="rounded-xl object-cover w-full aspect-video" />
          )}
        </div>
      </div>
    </section>
...
function SecaoPalestrantes({ dados }: { dados: any }) {
...
function SecaoAgenda({ dados }: { dados: any }) {
...
function SecaoDepoimentos({ dados }: { dados: any }) {
...
function SecaoLocal({ dados }: { dados: any }) {
...
function SecaoFaq({ dados }: { dados: any }) {
...
function SecaoCTA({ evento, onInscrever }: { evento: any; onInscrever: () => void }) {
  return (
...
const RENDERERS: Record<string, (s: Secao, evt: any, onInscrever: () => void) => JSX.Element | null> = {
  hero:         (s, e, cb) => <SecaoHero key={s.id} evento={e} dados={s.dados} onInscrever={cb} />,
  sobre:        (s) => <SecaoSobre key={s.id} dados={s.dados} />,
  palestrantes: (s) => <SecaoPalestrantes key={s.id} dados={s.dados} />,
  agenda:       (s) => <SecaoAgenda key={s.id} dados={s.dados} />,
  depoimentos:  (s) => <SecaoDepoimentos key={s.id} dados={s.dados} />,
  local:        (s) => <SecaoLocal key={s.id} dados={s.dados} />,
  faq:          (s) => <SecaoFaq key={s.id} dados={s.dados} />,
};
...
const EventoPublico = () => {
  ...
  const { data: evento, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await (supabase as any)
        .from("eventos")
        .select("id, nome, descricao, data, local, pago, valor, limite_participantes, tipo, status, banner_url, pagina_secoes, link_pagamento, pagina_publica_ativa")
        .eq("slug", slug)
  ...
  const { data: totalInscritos = 0 } = useQuery({
    queryKey: ["evento-publico-inscritos", evento?.id],
    queryFn: async () => {
  ...
  if (error || !evento) return (
    <div className="min-h-screen flex items-center justify-center text-center px-4">
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground text-sm">Este link pode ter expirado ou o evento não está disponível.</p>
      </div>
    </div>
  );

  const secoes: Secao[] = Array.isArray(evento.pagina_secoes)
    ? [...evento.pagina_secoes].filter((s) => s.ativo).sort((a, b) => a.ordem - b.ordem)
    : [];

  const temHero = secoes.some((s) => s.tipo === "hero");

  return (
    <div className="min-h-screen bg-background">
      {/* Se não tem seção hero configurada, mostra hero padrão */}
      {!temHero && (
        <SecaoHero evento={evento} dados={{ cta_texto: "Quero me inscrever" }} onInscrever={handleInscrever} />
      )}

      {secoes.map((secao) => {
        const render = RENDERERS[secao.tipo];
        return render ? render(secao, evento, handleInscrever) : null;
      })}

      {/* CTA final sempre aparece se evento está ativo */}
